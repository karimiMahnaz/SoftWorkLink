...
export const saveTask = async (email, attachedImage, projectId, ticket) => {
      ...
      if (attachedImage){
        formData.append(`${key}`, media);
           ...
           await  axios({
            ...
            console.log('http.url', `${http.url}/api/ticket/saveTask`)
        }
            axios({
                method: "post",
                url:`${http.url}/api/ticket/saveTask`,
                data: { email:email.replace(/"/g, ''), attachedImage , projectId, ticket}, 
            })
                .then( response => { console.log("Task update done."); return 200;})
                .catch(error =>{ console.log("Task update failed.", error); return 404;})
...
export const getTasks = async (email, projectId) => {
    try {

    
        const res = await http.get(
            `${http.url}/api/task/getTasks`,
            { params: {email, projectId}}
        );
        console.log('status', res.status);
        console.log('resdata0000', res.data);
        if (res.status === 200) {
            return  res.data;
        }
        return res.status;
...
export const getProjectTasks = async (email, projectId) => {
    try {
