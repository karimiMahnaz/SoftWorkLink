...
const DocumentPickerScreen = ({navigation}) => {
  ...
  const viewDocument = async e => {
    ...
    navigation.navigate('document', {
      pdf: {
        uri:
          `https://api.softestingca.com/uploaded-files/` +
          email.replace(/"/g, '').trim().toLowerCase() +
          `/` +
          e,
        name: e,
      ...
      <Screen>
        ...
        {!isSign && (
          <CustomText
            ...
            size="1.8"
            color="red"
            styles={styles.registerSignText}>
            Please register your sign before pick your document!
          </CustomText>
        )}
        <CustomText
          fontFamily="System"
        ...
        <CustomText
          ...
          File Name: {singleFile ? singleFile.name.substring(0, 60) : ''}
          {'\n\n'}
          Type: {singleFile ? singleFile.type.substring(0, 60) : ''}
          {'\n\n'}
          File Size: {singleFile ? singleFile.size : ''}
          {'\n\n'}
          URI: {singleFile ? singleFile.uri.substring(0, 50)+ '...' : ''}
          {'\n'}
        </CustomText>
        <View
          style={{
